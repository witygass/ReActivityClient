...
var fetchTemplate = function(user, callback, endpoint) {
  ...
  .then(function(response) {  
    ...
    a = JSON.parse(a);
    console.log('Data received! Data is:', a);
    callback(a);
  });
};
...
api.getNEvents = function(n, user, callback) {
  var lat = ~~(Math.random() * 20 + 50);
  var lon = ~~(Math.random() * -100 + 30);

  fetchTemplate(user, callback, baseUrl + '/api/activities/nearby/' + lat + '/' + lon + '/' + n);
}
