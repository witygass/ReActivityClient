...
api.filterEvents = function(eventList, options, callback) {
  ...
  if (eventList === 'nearbyEvents') eventArrType = 'nearby';
  ...
  if (sports) filterUrl += '?sportid=' + JSON.stringify(sports)
  ...
  AsyncStorage.getItem('JWTtoken').then((token) => {
    ...
    .catch(function(e) {
      console.log('There has been an error in API filterEvents. Error:', e);
    }) 
  })
}
